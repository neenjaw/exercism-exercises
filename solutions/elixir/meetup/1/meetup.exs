defmodule Meetup do
  ...
  defp get_initial_search_state(date, weekday, :last = schedule), 
    do: %{last_date_found: nil, initial_search_month: date.month} |> Map.merge(get_base_state(date, weekday, schedule))
  ...
  defp search(state) do
    ...
    case search_state do
      ...
      %{complete: false} -> search_state |> get_next_day_to_search |> search
    end
  end

  defp get_next_day_to_search(%{search_date: date} = state) do
    # Add one day to the date, then return the new state
    %{state | search_date: (date |> DateTime.add(86400, :second))}
  end
  ...
  # :first
  defp check_schedule(%{schedule: :first, num_weekday_found: n} = state) 
    when n < 1,  
    do: state
  defp check_schedule(%{schedule: :first, num_weekday_found: n} = state) 
    when n == 1, 
  ...
  # :second
  defp check_schedule(%{schedule: :second, num_weekday_found: n} = state) 
    when n < 2,  
    do: state
  defp check_schedule(%{schedule: :second, num_weekday_found: n} = state) 
    when n == 2, 
  ...
  # :third
  defp check_schedule(%{schedule: :third, num_weekday_found: n} = state) 
    when n < 3,  
    do: state
  defp check_schedule(%{schedule: :third, num_weekday_found: n} = state) 
    when n == 3, 
  ...
  # :fourth
  defp check_schedule(%{schedule: :fourth, num_weekday_found: n} = state) 
    when n < 4,  
    do: state
  defp check_schedule(%{schedule: :fourth, num_weekday_found: n} = state)
    when n == 4, 
  ...
  # :last
  defp check_schedule(%{schedule: :last, initial_search_month: m, search_date: (%DateTime{month: m} = date)} = state),
  ...
  # :teenth
  defp check_schedule(%{schedule: :teenth, search_date: %DateTime{day: d}} = state)
    when d in @teenth_date_range, 
    do: %{state | complete: true}
  defp check_schedule(%{schedule: :teenth} = state), do: state

  # get the weekday atom from the DateTime struct
  defp get_weekday_from_date(date) do 
    @weekday_int_to_atom
    |> Map.fetch!(Calendar.ISO.day_of_week(date.year, date.month, date.day))
  end
end
