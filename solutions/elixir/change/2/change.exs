defmodule Change do
  ...
  defp try_n_coins([head | tail], target, n) when head > target do
    try_n_coins(tail, target, n)
  end
  ...
  defp try_n_coins(coins = [coin | remaining_coins], target, n) do
    case try_n_coins(coins, target - coin, n - 1) do
      {:error, "cannot change"} -> try_n_coins(remaining_coins, target, n)
