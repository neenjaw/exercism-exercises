defmodule Tournament do
  ...
  defp get_away_outcome(home_outcome), do: home_outcome

  defp add_record(record_map, team, outcome) do
    team = Map.get(record_map, team, %Team{name: team})    

    updated_team = case outcome do
      "win"  -> Map.update(team, :wins, 0, &(&1 + 1)) |> Map.update(:points, 0, &(&1 + 3))
      "loss" -> Map.update(team, :loss, 0, &(&1 + 1))
      "draw" -> Map.update(team, :draw, 0, &(&1 + 1)) |> Map.update(:points, 0, &(&1 + 1))
    end
    |> Map.update(:matches, 0, &(&1 + 1))

    Map.put(record_map, team, updated_team_tally)
  end
  ...
  defp update_team(team, :match) do
    team
    |> Map.update(:matches, 0, &(&1 + 1))
  end

  defp format_tally(tallied_list_of_teams) do
    header = """
    Team                           | MP |  W |  D |  L |  P
    """

    tallied_list_of_teams
    |> Enum.map(&format_team(&1))
    |> Enum.join("\n")
    |> (fn body -> header <> body end).()
  end
