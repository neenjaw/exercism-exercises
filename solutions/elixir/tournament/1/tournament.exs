defmodule Tournament do
  ...
  defp add_record(record_map, team, outcome) do
    team_tally = Map.get(record_map, team, %Team{name: team})    

    updated_team_tally = case outcome do
      "win"  -> Map.update(team_tally, :wins, 0, &(&1 + 1)) |> Map.update(:points, 0, &(&1 + 3))
      "loss" -> Map.update(team_tally, :loss, 0, &(&1 + 1))
      "draw" -> Map.update(team_tally, :draw, 0, &(&1 + 1)) |> Map.update(:points, 0, &(&1 + 1))
    end
