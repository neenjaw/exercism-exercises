defmodule DiffieHellman do
  ...
  def generate_public_key(prime_p, prime_g, private_key) do
    public_key = :crypto.mod_pow(prime_g, private_key, prime_p)

    case public_key do
      key when is_binary(key) -> :binary.decode_unsigned(key)
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a) do
    shared_secret = :crypto.mod_pow(public_key_b, private_key_a, prime_p)

    case shared_secret do
      secret when is_binary(secret) -> :binary.decode_unsigned(secret)
      secret when is_integer(secret) -> secret
    end
  end
end
